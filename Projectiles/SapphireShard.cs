...
namespace GemstoneTools.Projectiles
{
    public class SapphireShard : ModProjectile
    {
		...
		public override void AI()
        {
			...
			{
				projectile.velocity.X = projectile.velocity.X * .8f;
				xVelocityChecker = 1;
			}
			if (yVelocityChecker == 0)
			{
				projectile.velocity.Y = projectile.velocity.Y * .8f;
				yVelocityChecker = 1;
			}

			//Go up for the first 15 frames after the projectile spawns
			if (tickCounter < 0)
			{
			...
			//Create a dust trail
			Dust.NewDust(projectile.position, projectile.width + 10, projectile.height + 10, mod.DustType("SapphireSparkle"), projectile.velocity.X * 0.25f, projectile.velocity.Y * 0.25f, 150, default(Color), 0.7f);

			//Create light
			Lighting.AddLight(projectile.position, 0, .3f, 1); //Position, R,G,B
		}
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			int randInt = Main.rand.Next(1, 11); //40% chance to slow
			if (randInt <= 4)
				target.AddBuff(32, 600, true);
		}
