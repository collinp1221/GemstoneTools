...
{
	public class EmeraldShurikenProjectile : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if(tickCounter >= 15)
            {
				Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType("EmeraldSparkle"), projectile.velocity.X * 0.25f, projectile.velocity.Y * 0.25f, 150, default(Color), 0.7f);
				tickCounter = 0;
			}

			//Create light centered on the shuriken
			Lighting.AddLight(projectile.position, 0, .4f, .2f); //Position, R,G,B
		}
		
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			//Create a particle effect on hitting an NPC
			Dust.NewDust(projectile.position, projectile.width, projectile.height, 140);
		}
		...
		public override void Kill(int timeLeft)
		{
			...
			if (randInt >= 8)
				Item.NewItem(projectile.getRect(), mod.ItemType("EmeraldShuriken"));
		}
