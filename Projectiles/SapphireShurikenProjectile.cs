...
{
	public class SapphireShurikenProjectile : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if(tickCounter >= 15)
            {
				Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType("SapphireSparkle"), projectile.velocity.X * 0.25f, projectile.velocity.Y * 0.25f, 150, default(Color), 0.7f);
				tickCounter = 0;
			}

			Lighting.AddLight(projectile.position, 0, .15f, .5f); //Position, R,G,B
		}
		
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			//Create a particle effect on hitting an NPC
			Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType("SapphireSparkle"));
		...
		public override void Kill(int timeLeft)
		{
			...
			if (randInt >= 8)
				Item.NewItem(projectile.getRect(), mod.ItemType("SapphireShuriken"));
		}
