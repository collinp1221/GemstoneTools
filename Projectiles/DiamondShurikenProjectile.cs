...
{
	public class DiamondShurikenProjectile : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if(tickCounter >= 15)
            {
				Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType("DiamondSparkle"), projectile.velocity.X * 0.25f, projectile.velocity.Y * 0.25f, 150, default(Color), 0.7f);
				tickCounter = 0;
			}
		}
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType("DiamondSparkle"));
		}
		...
		{
			//30% Chance to drop the shuriken on despawn
			int randInt = Main.rand.Next(1, 11);
			if (randInt >= 8)
				Item.NewItem(projectile.getRect(), mod.ItemType("DiamondShuriken"));
		}
