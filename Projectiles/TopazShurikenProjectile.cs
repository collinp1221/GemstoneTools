...
{
	public class TopazShurikenProjectile : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if(tickCounter >= 15)
            {
				Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType("TopazSparkle"), projectile.velocity.X * 0.25f, projectile.velocity.Y * 0.25f, 150, default(Color), 0.7f);
				tickCounter = 0;
			}

			Lighting.AddLight(projectile.position, .4f, .25f, 0); //Position, R,G,B
		}
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType("TopazSparkle"));
		}
		...
		public override bool OnTileCollide(Vector2 velocityChange)
		{
			...
			//Play a sound when the shuriken collides with a tile
			Main.PlaySound(0);
		...
		{
			//30% Chance to drop the shuriken on despawn (for consistency)
			int randInt = Main.rand.Next(1, 11);
			if (randInt >= 8)
				Item.NewItem(projectile.getRect(), mod.ItemType("TopazShuriken"));
		}
